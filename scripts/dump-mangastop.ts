...
async function main() {
  console.log("")
  console.log("╔══════════════════════════════════════════════╗")
  console.log("║   Dump do Catálogo MangaStop.net            ║")
  console.log("╚══════════════════════════════════════════════╝")
  console.log("")
  ...
  const slugs = await getAllMangaStopSlugs()
  ...
  for (let i = 0; i < slugs.length; i++) {
    ...
    try {
      const manga = await getMangaStopCached(slug)
      await sleep(RATE_LIMIT_MS)

      const chapters = await getMangaStopChaptersCached(slug)
      await sleep(RATE_LIMIT_MS)
      ...
      success++
      if ((i + 1) % 50 === 0 || i === 0) {
        const total = await getCatalogCount("mangastop")
